...
{
	$busqueda="";
	$codigo_producto=$_REQUEST["id_producto"];
...
?>
	...
	<div class="form-group">
		<div class="controls">
			<input id="numeroserie" class="form-control" type="hidden" placeholder="Numero de Serie" size="16" value="<?php if(isset($_SESSION['detalle_facturacion'][(string)$respuesta['Pro_Codigo']])){?><?php echo $_SESSION['detalle_facturacion'][(string)$respuesta['Pro_Codigo']]['numeroserie'];?><?php }?>" onkeyup="onlyNumbers(this);">
		</div>
	</div>
	<div class="form-group">
		<div class="controls">
			<input id="numerolinea" class="form-control" type="hidden" placeholder="Numero de Linea" size="16"  value="<?php if(isset($_SESSION['detalle_facturacion'][(string)$respuesta['Pro_Codigo']])){?><?php echo $_SESSION['detalle_facturacion'][(string)$respuesta['Pro_Codigo']]['numerolinea'];?><?php }?>" onkeyup="onlyNumbers(this);">
		</div>
	</div>
	<div class="form-group">
		<div class="controls">
			<button <?php if(isset($_SESSION['detalle_facturacion'][(string)$respuesta['Pro_Codigo']])){?>onclick="Editar_Detalle('<?php echo $codigo_producto;?>','<?php echo $_SESSION['detalle_facturacion'][(string)$respuesta['Pro_Codigo']]['id_html'];?>')"<?php }else{ ?>onclick="Agregar_Detalle('<?php echo $codigo_producto;?>')"<?php } ?> type="button" class="boton_save"><i class="fa fa-plus-square"></i><?php if(isset($_SESSION['detalle_facturacion'][(string)$respuesta['Pro_Codigo']])){?>Editar<?php }else{?>Agregar<?php }?></button> 
		</div>
	</div>
	<?php }?>
<?php } ?>
