...
?>
	...
	<div class="form-group">
		<label>Valor</label>
		<div class="controls">
			<input id="valor" class="form-control" type="text" placeholder="Valor" size="16" value="<?php echo $respuesta['Pro_Costo'];?>">
		</div>
	</div>
	<div class="form-group">
		<div class="controls">
	...
	<div class="form-group">
		<div class="controls">
			<button <?php if(isset($_SESSION['detalle_facturacion'][(string)$respuesta['Pro_Codigo']])){?>onclick="Editar_DetalleI('<?php echo $respuesta['DProd_Codigo'];?>','<?php echo $_SESSION['detalle_facturacion'][(string)$respuesta['Pro_Codigo']]['id_html'];?>')"<?php }else{ ?>onclick="Agregar_DetalleI('<?php echo $respuesta['DProd_Codigo'];?>')"<?php } ?> type="button" class="boton_save"><i class="fa fa-plus-square"></i><?php if(isset($_SESSION['detalle_facturacion'][(string)$respuesta['Pro_Codigo']])){?>Editar<?php }else{?>Agregar<?php }?></button> 
		</div>
	</div>
	<?php }?>
<?php } ?>
